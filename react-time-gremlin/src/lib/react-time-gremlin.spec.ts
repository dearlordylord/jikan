...
it('measures irregular wake-ups and uses the latest consumer dispatch without rescheduling', () => {
  ...
  const schedule = jest.fn((callback: () => void) => { wake = callback; return cleanup; });
  const now = () => nowMs;
  const dispatch = jest.fn();
  const latestDispatch = jest.fn();
  const initial = { uiState: running(), setUiState: jest.fn(), dispatch, now, schedule };
  const { rerender, unmount } = renderHook(useTimeGremlin, { initialProps: initial });
  act(() => { nowMs = 0.6; wake(); });
  expect(dispatch).not.toHaveBeenCalled();
  rerender({ ...initial, dispatch: latestDispatch });
  act(() => { nowMs = 4000.2; wake(); });
...
it('flushes before pause, preserves fractional carry, and excludes paused time on resume', () => {
  let nowMs = 0;
  let wake: () => void = () => undefined;
  const schedule = (callback: () => void) => { wake = callback; return () => undefined; };
  const now = () => nowMs;
  const dispatch = jest.fn();
  const state = running();
  const initial = { uiState: state, setUiState: jest.fn(), dispatch, now, schedule };
  const { result, rerender } = renderHook(useTimeGremlin, { initialProps: initial });
  act(() => { nowMs = 400.8; result.current.pause(); });
...
it('keeps deterministic manual input available and rejects bad clocks without model delivery', () => {
  ...
  const { result } = renderHook(() => useTimeGremlin({
    uiState: running(), setUiState: jest.fn(), dispatch, onIssues,
    now: () => nowMs, schedule,
  }));
  act(() => { nowMs = Number.NaN; wake(); });
...
it('ignores a deterministic callback canceled by restart or unmount', () => {
  ...
  const schedule = (callback: () => void) => { callbacks.push(callback); return () => undefined; };
  const now = () => 0;
  const dispatch = jest.fn();
  const { result, unmount } = renderHook(() => useTimeGremlin({
    uiState: running(), setUiState: jest.fn(), dispatch, now, schedule, appetite: BigInt(10),
  }));
...
it('delivers fallback transition facts exactly once after committing state', () => {
  ...
  const initial = { uiState: state, setUiState, now: () => sample, schedule, onTransition };
  const { rerender } = renderHook(useTimeGremlin, { initialProps: initial });
  act(() => { sample = 4000; wake(); });
  expect(setUiState).toHaveBeenCalledTimes(1);
  expect(onTransition).toHaveBeenCalledWith([{ kind: 'warmup', duration: 3000 }]);
  expect(setUiState.mock.invocationCallOrder[0]).toBeLessThan(onTransition.mock.invocationCallOrder[0]);
  rerender({ ...initial, uiState: setUiState.mock.calls[0][0] });
...
it('rejects scheduler delays beyond the platform timer bound', () => {
  ...
  renderHook(() => useTimeGremlin({
    uiState: running(), setUiState: jest.fn(), speed: BigInt(2147483648), onIssues,
  }));
  expect(interval).not.toHaveBeenCalled();
  expect(onIssues).toHaveBeenCalledWith([expect.objectContaining({ path: 'speed', code: 'invalid-cadence' })]);
