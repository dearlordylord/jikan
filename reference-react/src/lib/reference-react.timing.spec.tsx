...
const fixture = () => {
  ...
  const schedule = jest.fn((wake: () => void) => { callbacks.push(wake); return cleanup; });
  return { timing: { now: () => value, schedule }, callbacks, cleanup, setNow: (next: number) => { value = next; } };
};
const setupProgram = () => {
  fireEvent.change(screen.getByLabelText('rounds:'), { target: { value: '3' } });
  fireEvent.change(screen.getByLabelText('exercise time ms:'), { target: { value: '100' } });
  fireEvent.change(screen.getByLabelText('rest time ms:'), { target: { value: '0' } });
...
it('uses the real measured path and delivers ordered identical stages once in StrictMode', () => {
  ...
  const { rerender, unmount } = render(<StrictMode><ReferenceReact timing={clock.timing} onTransition={onTransition} /></StrictMode>);
  setupProgram();
  expect(clock.timing.schedule).toHaveBeenCalledTimes(1);
  act(() => { clock.setNow(0.6); clock.callbacks[0](); });
  expect(onTransition).not.toHaveBeenCalled();
  const latestTransition = jest.fn();
  rerender(<StrictMode><ReferenceReact timing={clock.timing} onTransition={latestTransition} /></StrictMode>);
  act(() => { clock.setNow(3200.2); clock.callbacks[0](); });
  expect(screen.getByRole('status').textContent).toContain('Round 3 of 3');
  expect(onTransition).not.toHaveBeenCalled();
  expect(latestTransition).toHaveBeenCalledTimes(1);
  expect(latestTransition.mock.calls[0][0]).toEqual([
...
it('flushes before pause, resumes against current workout state and rejects stale stopped sessions', () => {
  ...
  expect(screen.getByRole('status').textContent).toContain('Paused: Round 2 of 3');
  expect(onTransition.mock.calls[0][0]).toHaveLength(2);
  clock.setNow(5000);
  fireEvent.click(screen.getByText('Continue'));
  act(() => { clock.setNow(5000.3); clock.callbacks[1](); });
  expect(screen.getByRole('status').textContent).toContain('99 ms remaining');
  fireEvent.click(screen.getByText('Stop'));
  const stoppedCallback = clock.callbacks[1];
  clock.setNow(9000);
  fireEvent.click(screen.getByText('Start'));
  act(() => stoppedCallback());
  ...
  const active = clock.callbacks[clock.callbacks.length - 1];
  act(() => { clock.setNow(12000); active(); });
...
it('rejected settings and clock samples produce no successful transition effects', () => {
  ...
  expect(onTransition).not.toHaveBeenCalled();
  setupProgram();
  act(() => { clock.setNow(Number.NaN); clock.callbacks[0](); });
