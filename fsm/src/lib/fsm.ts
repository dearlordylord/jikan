...
export const showPrintDuration0QueueItemError = (queueItem: QueueItem<string>): string => `duration <= 0 queue item provided: ${showQueueItem(queueItem)}; this is not an error, but it is not recommended; the item will be ignored`;
...
const showQueueItem = <Kind extends string>(queueItem: QueueItem<Kind>): string => `${queueItem.kind}(${queueItem.duration})`;
...
export const empty = <Kind extends string>(): State<Kind> => empty_ as State<Kind>;

export const isEmpty = <Kind extends string>(state: State<Kind>): state is EmptyState => state.queue.length === 0 && assertTrue/*defensive*/(state.duration === 0);
...
export const push = <Kind_ extends string>(qx_: Queue<Kind_>) => <Kind extends string>(state: State<Kind_ extends Kind ? Kind : never>): State<Kind | Kind_> => {
  const qx = Object.freeze/*just to cast to RA*/(qx_.filter((x) => {
...
export const pop = <Kind extends string>(state: State<Kind>): [State<Kind>, QueueItem<Kind> | null] => {
...
export const currentNE = <Kind extends string>(state: NonEmptyState<Kind>): QueueItem<Kind> => lastNEA(state.queue);

export const current = <Kind extends string>(state: State<Kind>): QueueItem<Kind> | null => isEmpty(state) ? null : currentNE(state);

export const tick = (step: number/*no check, because 0s and negatives are all right, theoretically*/) => <Kind extends string>(state: State<Kind>): [State<Kind>, QueueItem<Kind>[]] => {
  const tick_ = <Kind extends string>(step: number, state: State<Kind>, acc: QueueItem<Kind>[]): [State<Kind>, QueueItem<Kind>[]] => {
